...
/* These types must be visible in the header so mp3dec_t can embed mp3dec_scratch_t */
#ifndef MAX_FREE_FORMAT_FRAME_SIZE
...
typedef struct { const uint8_t *buf; int pos, limit; } mp3dec_bs_hdr_t;
...
} mp3dec_gr_info_hdr_t;

typedef struct
{
    mp3dec_bs_hdr_t bs;
    uint8_t maindata[MAX_BITRESERVOIR_BYTES + MAX_L3_FRAME_PAYLOAD_BYTES];
    mp3dec_gr_info_hdr_t gr_info[4];
...
typedef struct
{
    ...
    mp3dec_scratch_t scratch; /* heap-allocated scratch (avoids 13KB stack in 16-bit DLL callbacks) */
} mp3dec_t;
...
typedef mp3dec_gr_info_hdr_t L3_gr_info_t;
...
{
    unsigned long _fb = (unsigned long)hdr_frame_samples(h) * (unsigned long)hdr_bitrate_kbps(h) * 125UL / (unsigned long)hdr_sample_rate_hz(h);
    int frame_bytes = (int)_fb;
    if (HDR_IS_LAYER_1(h))
    {
...
#define DQ(x) 9.53674316e-07f/x, 7.56931807e-07f/x, 6.00777173e-07f/x
    ...
    for (i = 0; i < bands; i++)
    {
        ...
        for (m = 4; m; m >>= 1)
        {
            ...
            {
                int b = get_bits(bs, 6);
                s = g_deq_L12[ba*3 - 6 + b % 3]*((long)1 << 21 >> b/3);
            }
            *scf++ = s;
        }
    }
...
static int L3_read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    int sr_idx, gr_count;
    sr_idx = HDR_GET_MY_SAMPLE_RATE(hdr); sr_idx -= (sr_idx != 0);
    gr_count = HDR_IS_MONO(hdr) ? 1 : 2;
...
#define BSPOS         ((bs_next_ptr - bs->buf)*8 - 24 + bs_sh)
    ...
    uint32_t bs_cache = (((uint32_t)bs_next_ptr[0] << 24) | ((uint32_t)bs_next_ptr[1] << 16) | ((uint32_t)bs_next_ptr[2] << 8) | bs_next_ptr[3]) << (bs->pos & 7);
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    const uint8_t *hdr;
    bs_t bs_frame[1];
    mp3dec_scratch_t *scratch = &dec->scratch;
...
/* Decode one MP2/MP3 frame of KNOWN size - bypasses frame sync detection.
 * Use when the caller has already parsed the header and computed the exact
 * frame size using 32-bit arithmetic (avoids 16-bit overflow in hdr_frame_bytes).
 * mp3 must point to start of a valid frame header; mp3_bytes = exact frame size. */
static int mp3dec_decode_known_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    int i, igr, success = 1;
    const uint8_t *hdr = mp3;
    bs_t bs_frame[1];
    mp3dec_scratch_t *scratch = &dec->scratch;
    ...
    bs_init(bs_frame, hdr + HDR_SIZE, mp3_bytes - HDR_SIZE);
    if (HDR_IS_CRC(hdr))
        get_bits(bs_frame, 16);
    ...
    {
        int main_data_begin = L3_read_side_info(bs_frame, scratch->gr_info, hdr);
        if (main_data_begin < 0) { mp3dec_init(dec); return 0; }
        if (bs_frame->pos > bs_frame->limit) { mp3dec_init(dec); return 0; }
